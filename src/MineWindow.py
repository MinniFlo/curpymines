...
class MineWindow(SuperWin):
    ...
    def render_fields_from_render_list(self):
        for coordinates in self.logic.render_list:
            if coordinates in self.logic.game_grid.boarder:
                continue
            # FC is important: field is_relevant gets set
    ...
    def render_loose(self):
        grid_iter = iter(self.logic.game_grid)
        ...
        curs_field = self.logic.game_grid.get_field((self.cursor_y, self.x_index))
        # on mine
    ...
    def reset_render(self):
        ...
        for field in grid_iter:
            y_pos, x_pos = field.render_coordinates
            self.scr.addstr(y_pos, x_pos, field.symbol, curses.color_pair(field.color_id))
        self.scr.box()
